...
namespace WebAPI_IE307Final.Controllers
{
    public class HelloWebAPIController : ApiController
    {
       ...
       public IHttpActionResult GameList()
        ...
        [Route("api/HelloWebAPIController/AddToFav")]
        [HttpPost]
        public IHttpActionResult AddToFav(Favourite crt)
        {
            try
            {
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("pdid", crt.ProductID);
                param.Add("usrID", crt.UserID);
                int kq = int.Parse(Database.Exec_Command("AddToFav", param).ToString());
                string kqtv = Database.Exec_Command("AddToFav", param).ToString();
                if (kqtv == null || kq < 0) crt.UserID = kq;
                return
                 Ok(crt);
            }
        ...
        [Route("api/HelloWebAPIController/Load_Fav")]
        [HttpGet]
        public IHttpActionResult Load_Fav(int userID)
        {
            try
            {
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("usrID", userID);
                DataTable tb = Database.Read_Table("Load_Fav", param);
                return Ok(tb);
            }
            catch
            {
        ...
        [Route("api/HelloWebAPIController/Cart_Delete")]
        [HttpPost]
        public IHttpActionResult Cart_Delete(Cart crt)
        {
            try
            {
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("usrID", crt.UserID);
                param.Add("pdid", crt.ProductID);
                int kq = int.Parse(Database.Exec_Command("Cart_Delete", param).ToString());
                if (kq <= 0) crt.ProductID = kq;
                return Ok(crt);
            }
            catch
            {
        ...
        [Route("api/HelloWebAPIController/Fav_Delete")]
        [HttpPost]
        public IHttpActionResult Fav_Delete(Favourite crt)
        {
            try
            {
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("usrID", crt.UserID);
                param.Add("pdid", crt.ProductID);
                int kq = int.Parse(Database.Exec_Command("Fav_Delete", param).ToString());
                if (kq <= 0) crt.ProductID = kq;
                return Ok(crt);
            }
            catch
